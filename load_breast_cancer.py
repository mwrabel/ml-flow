...
x, y = make_classification(n_samples=5000, n_features=100, n_informative=10, n_redundant=10, n_repeated=5, flip_y=0.01, n_classes=2, class_sep=0.6, random_state=10)
...
clf = RandomizedSearchCV(pipe, param_grid, cv=cv, verbose=1, n_iter=20, scoring='roc_auc', n_jobs=-1)
...
clf_halving = HalvingRandomSearchCV(pipe, param_grid, cv=cv, verbose=1, scoring='roc_auc', n_jobs=-1,
                                    aggressive_elimination=True, factor=3, min_resources=50)
clf_halving = clf_halving.fit(x, y)
# Score model
